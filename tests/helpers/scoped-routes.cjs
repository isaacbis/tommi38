...
module.exports=function setup(file='account-routes.js'){
 ...
 const context=vm.createContext({db,root:{...db,collection},z,bcrypt,randomInt:require('node:crypto').randomInt,Buffer,console,Date,Intl,tenantId:()=>scope.getStore(),FieldValue:{increment:n=>({increment:n}),serverTimestamp:()=>0},express:{Router:()=>router},rateLimit:()=> (req,res,next)=>next(),establishments:async()=>[]});
