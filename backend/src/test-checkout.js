// Sandbox-only payment core; never accepts live credentials.
  ...
  return {
    ...
    async fulfillVerifiedEvent(event) {
      ...
      return db.runTransaction(async tx=>{
        ...
        if(session.id!==value.sessionId || event.account!==value.account || session.amount_total!==value.priceCents || session.currency!=='eur')return false;
